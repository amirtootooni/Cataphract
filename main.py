...
def trainDecisionTree(X, y, max_depth=4, step_size=1):
...
def trainRandomForrest(X, y, max_depth=4, step_size=1):
    depths = range(1, max_depth+1, step_size)
    scores = np.empty(len(depths))
...
def trainKNN(X, y, max_k=8, step_size=1):
...
def trainNeuralNetwork(X,y):
    model = MLPClassifier(learning_rate='adaptive', early_stopping=True, max_iter=100, batch_size=500)
    model = model.fit(X,y)
    return model, 0
...
def analyzeModels(num_models, training_size, test_size, num_tasks):
    ...
    def analyzeModel(training_size, test_size, num_tasks, training_errors, test_errors, hyperbolic_errors, i):
        ...
        for j in range(4):
            print('Thread ', i, ' working on model ', j)
...
# analyzeModels(10, 10000, 2000, 16)
